...
{	
  ...
  LOOM_DEBUG_Println2("LARGER BUF: ", strlen(larger_buf));
  ...
  LOOM_DEBUG_Println2("COMPRESSED BUF: ", strlen(larger_buf));
...
//		sprintf(address_string, "%s%d/%s%d%s", FAMILY, FAMILY_NUM, DEVICE, INIT_INST, key);
    sprintf(address_string, "%s/%s", "a", key);
...
void append_to_bundle_key_value(OSCBundle *bndl, char* key, unsigned long elem)
{
  char address_string[255];
  append_to_bundle_key_value_aux(bndl, address_string, key);
  bndl->add(address_string).add( (int32_t)elem );
